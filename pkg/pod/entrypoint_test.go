...
func TestStopSidecars(t *testing.T) {
	...
	// This is a container that is added by the controller for accessing sidecar logs.
	// This should not be stopped as long as results-from is set to sidecar-logs.
	artifactsSidecar := corev1.Container{
		Name:  pipeline.ReservedArtifactsSidecarContainerName,
		Image: "original-injected-image",
	}
	// This container can be stopped if the results-from is not set to sidecar-logs.
	stoppedArtifactsSidecar := corev1.Container{
		Name:  pipeline.ReservedArtifactsSidecarContainerName,
		Image: nopImage,
	}
	...
	for _, c := range []struct {
		...
		desc: "Artifacts Sidecar should not be stopped",
		pod: corev1.Pod{
			ObjectMeta: metav1.ObjectMeta{
				Name: "test-pod",
			},
			Spec: corev1.PodSpec{
				Containers: []corev1.Container{stepContainer, sidecarContainer, artifactsSidecar},
			},
			Status: corev1.PodStatus{
				Phase: corev1.PodRunning,
				ContainerStatuses: []corev1.ContainerStatus{{
					...
					Name: artifactsSidecar.Name,
					// Artifacts sidecar is running.
					State: corev1.ContainerState{Running: &corev1.ContainerStateRunning{StartedAt: metav1.NewTime(time.Now())}},
				}},
			},
		},
		resultExtractionMethod: "sidecar-logs",
		wantContainers:         []corev1.Container{stepContainer, stoppedSidecarContainer, artifactsSidecar},
	}, {
		desc: "Artifacts Sidecar should be stopped result method is not sidecar logs",
		pod: corev1.Pod{
			ObjectMeta: metav1.ObjectMeta{
				Name: "test-pod",
			},
		...
		wantContainers: []corev1.Container{stepContainer, stoppedSidecarContainer, stoppedArtifactsSidecar},
	}, {
